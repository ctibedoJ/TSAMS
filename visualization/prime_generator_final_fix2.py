#!/usr/bin/env python3
"""
Final Fix TSAMS Prime Generator Formula Visualizer
Creates a visualization with absolutely no text overlap.
"""
...
# Create a much larger figure to completely eliminate overlap
fig = plt.figure(figsize=(20, 12))
fig.patch.set_facecolor('#f8f9fa')

# Create a grid layout with completely separate areas for each component
gs = gridspec.GridSpec(2, 2, figure=fig, height_ratios=[1, 1.5], width_ratios=[1, 1.5])

# Create subplots
ax1 = fig.add_subplot(gs[0, 0])  # Formula visualization
ax2 = fig.add_subplot(gs[0, 1])  # Prime sequence
ax3 = fig.add_subplot(gs[1, 0], projection='3d')  # 3D visualization for coupling strengths - FIXED
ax4 = fig.add_subplot(gs[1, 1])  # Prime generation process - MUCH MORE SPACE

# Set titles
fig.suptitle('TSAMS Prime Generator Formula Visualization', fontsize=24, fontweight='bold', y=0.98)
ax1.set_title('Prime Generator Formula', fontsize=20)
ax2.set_title('Prime Number Sequence', fontsize=20)
ax4.set_title('Prime Generation Process', fontsize=20)
ax3.set_title('Coupling Visualization', fontsize=18)

# Parameters - reduced for faster processing
max_n = 100  # Maximum n-th prime to visualize
...
def generate_nodal_points(n, k, time_factor=0):
    ...
    # Generate points on a cube - limit the number of points for performance
    k = min(k, 10)  # Limit k to avoid too many points
...
print("Pre-calculating primes and formula steps...")
for n in range(1, max_n + 1):
    p = tsams_prime_generator(n)
    primes.append(p)
    n_values.append(n)
    formula_steps.append(formula_calculation_steps(n))
print(f"Pre-calculation complete. Generated {len(primes)} primes.")
...
def update(frame):
    ...
    # Plot 1: Formula Visualization - COMPLETELY SEPARATE
    ax1.axis('off')
    
    # Much more vertical spacing between formula components
    formula = r"$P(n) = 1 + \sum_{k=1}^{\lfloor\sqrt{F(n)}\rfloor} \lfloor\cos^2(\pi\cdot\Gamma(n,k)/k)\rfloor$"
    ax1.text(0.5, 0.85, formula, fontsize=18, ha='center', va='center')
    
    # Add explanation of the formula components with MUCH more spacing
    explanation = [
        r"Where:",
    ...
    for i, line in enumerate(explanation):
        ax1.text(0.5, 0.65 - i*0.15, line, fontsize=16, ha='center', va='center')
    
    # Highlight current calculation - moved to bottom with plenty of space
    ax1.text(0.5, 0.05, f"Currently calculating: P({current_n}) = {primes[current_n-1]}", 
    ...
    ax2.set_ylabel('P(n)', fontsize=16)
    ...
    # Plot 3: 3D Visualization for coupling strengths and nodal alignments
    # Set up the 3D axes
    ax3.set_xlim(-1.2, 1.2)
    ax3.set_ylim(-1.2, 1.2)
    ax3.set_zlim(-1.2, 1.2)
    ax3.set_box_aspect([1, 1, 1])  # Make the box cubic
    ...
    k_value = 0
    for step in formula_steps[current_n-1]:
    ...
    if nodal_points:
        ...
        ax3.scatter(xs, ys, zs, c=nodal_colors, cmap='plasma', 
    ...
    for x, y, size, color_val in circles:
        circle = plt.Circle((x, y), size, color=plt.cm.viridis(color_val), 
                           alpha=0.7, transform=ax3.transData)
        # Project onto the bottom face
        z = -1
        ax3.add_patch(circle)
        art3d.pathpatch_2d_to_3d(circle, z=z, zdir="z")
    
    # Add coupling lines between nodal points - limit for performance
    max_lines = 50
    line_count = 0
    ...
    if len(nodal_points) >= 2:
        for i in range(len(nodal_points)-1):
            for j in range(i+1, len(nodal_points)):
                ...
                if strength > 0.5 and line_count < max_lines:  # Higher threshold and limit
    ...
    ax3.text2D(0.05, 0.95, f"k={k_value}", transform=ax3.transAxes,
              fontsize=12, ha='left', va='top',
              bbox=dict(facecolor='white', alpha=0.7, boxstyle='round'))
    
    ax3.text2D(0.05, 0.05, "Nodal Alignments", transform=ax3.transAxes,
              fontsize=12, ha='left', va='bottom',
              bbox=dict(facecolor='white', alpha=0.7, boxstyle='round'))
    
    # Plot 4: Formula Calculation Steps - COMPLETELY REDESIGNED WITH NO OVERLAP
    ax4.axis('off')
    
    # Display the calculation steps for the current n
    current_steps = formula_steps[current_n-1]
    
    # Always show all steps - no animation for steps
    steps_to_show = len(current_steps)
    
    # SINGLE COLUMN LAYOUT - VERTICAL ONLY - NO OVERLAP POSSIBLE
    # Calculate font size based on number of steps
    step_font_size = 14  # Larger font size
    
    # Calculate vertical spacing based on number of steps
    vertical_spacing = 0.9 / (len(current_steps) + 1)  # Distribute steps evenly
    
    for i, step in enumerate(current_steps[:steps_to_show]):
        # Calculate position with plenty of vertical space
        y_pos = 0.95 - (i + 1) * vertical_spacing
        
        # Highlight the final step
        if i == len(current_steps) - 1:
            ax4.text(0.05, y_pos, step, fontsize=step_font_size+2, ha='left', va='center',
                    bbox=dict(facecolor='lightgreen', alpha=0.5, boxstyle='round'))
        else:
            ax4.text(0.05, y_pos, step, fontsize=step_font_size, ha='left', va='center')
    
    # Calculate progress
    progress = current_n / max_n
    
    # Add progress indicator at the very bottom
    ax4.add_patch(plt.Rectangle((0.1, 0.05), progress * 0.8, 0.03, 
                              facecolor='blue', alpha=0.7))
    ax4.text(0.5, 0.02, f"Progress: {current_n}/{max_n} primes", 
             ha='center', va='center', fontsize=14)
    
    # Use a very generous spacing for the layout
    plt.subplots_adjust(left=0.05, right=0.95, bottom=0.05, top=0.92, wspace=0.3, hspace=0.3)
    
    print(f"Processed frame {frame+1}/{frames} for n={current_n}")
...
# Save as MP4
ani.save('tsams_prime_generator_fixed.mp4', writer='ffmpeg', fps=fps, dpi=150, bitrate=5000)

print("Saving final frame...")
# Display final frame
update(frames-1)
plt.savefig('tsams_prime_generator_fixed_final_frame.png', dpi=300)
print("Fixed visualization complete. Video saved as 'tsams_prime_generator_fixed.mp4'")